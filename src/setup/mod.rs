...
pub fn setup(config_file: String, build_dir: String, prefix: String, options: Vec<(String, String)>, dependency_overrides: Vec<(String, String)>) -> Result<()> {
    ...
    // Resolve config
    let config_file = Path::new(&config_file).canonicalize().context("Failed to canonicalize config path")?;
    let config_dir = match config_file.parent() {
        None => bail!("Failed to resolve config directory"),
        Some(config_dir) => config_dir,
    };
    let config_file = diff_paths(&config_file, config_dir).unwrap();

    // Change working directory into config_dir
    set_current_dir(config_dir).with_context(|| format!("Failed to change into config directory `{}`", config_dir.to_str().unwrap()))?;
    ...
    // Write gitignore
    fs::write(build_dir.join(".gitignore"), "# Generated by Fabricate\n*").context("Failed to write .gitignore")?;
    ...
    // Build ninja file
    fs::write(build_dir.join("build.ninja"), build_ninja_file(&rules, &builds)).context("Failed to write ninja build file")?;
    ...
    if let Some(ninja_path) = ninja_path {
        ...
        if compdb_rules.len() > 0 {
            ...
            fs::write(build_dir.join("compile_commands.json"), output.stdout).context("Failed to write compile_commands.json")?;
        }
    }
